...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .medal-ring {
      position:relative; width:172px; height:172px; border-radius:50%; overflow:hidden;
      display:flex; flex-direction:column; align-items:center; justify-content:center;
      background:radial-gradient(circle at 34% 28%, #ffedb0, #d4a017 58%, #a86a06 100%);
      box-shadow:0 0 0 6px var(--gold-glow), 0 14px 34px rgba(0,0,0,.3);
      animation:popIn .65s cubic-bezier(.34,1.56,.64,1) both;
    }
    .medal-ring::after {
      content:''; position:absolute; top:8%; left:14%; width:46%; height:30%; border-radius:50%;
      background:rgba(255,255,255,.35); filter:blur(6px); transform:rotate(-18deg);
    }
    .medal-total { font-family:'Aref Ruqaa','Cairo',serif; font-weight:700; font-size:56px; line-height:1; color:#4a2a06; animation:numRise .5s .3s cubic-bezier(.16,1,.3,1) both; position:relative; z-index:1; }
    .medal-of { font-size:11.5px; color:rgba(74,42,6,.65); margin-top:2px; position:relative; z-index:1; }
    .medal-seal {
