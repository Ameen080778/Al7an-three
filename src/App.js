...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ═══════════════════════════════════════════════
       COPTIC CANDLE — شمعة الدرجة المتقدة (progress candle)
    ═══════════════════════════════════════════════ */
    .candle-wrap { position:relative; display:flex; flex-direction:column; align-items:center; padding:26px 10px 6px; }

    /* الإطار الزخرفي القبطي الدائري */
    .candle-halo {
      position:absolute; top:36px; left:50%; width:246px; height:246px; transform:translate(-50%,0);
      border-radius:50%; pointer-events:none;
      background:
        repeating-conic-gradient(from 0deg, var(--gold-glow) 0deg 1.2deg, transparent 1.2deg 9deg);
      animation: haloTurn 40s linear infinite;
    }
    @keyframes haloTurn { to { transform:translate(-50%,0) rotate(360deg); } }
    .candle-halo-ring {
      position:absolute; top:36px; left:50%; width:210px; height:210px; transform:translate(-50%,0);
      border-radius:50%; border:1px dashed var(--border-g); pointer-events:none;
    }
    .candle-cross {
      position:absolute; width:13px; height:13px; opacity:.85; color:var(--gold);
    }

    /* منصّة/قاعدة الشمعة */
    .candle-stage {
      position:relative; width:220px; height:250px; display:flex; flex-direction:column; align-items:center; justify-content:flex-end;
    }
    .candle-base {
      width:96px; height:14px; border-radius:50%;
      background:radial-gradient(ellipse at center, var(--gold-glow), transparent 75%);
      margin-top:-6px;
    }
    .candle-body-wrap {
      position:relative; width:74px; height:172px; display:flex; align-items:flex-end; justify-content:center;
    }
    /* جسم الشمعة الزجاجي (الوعاء) */
    .candle-glass {
      position:relative; width:74px; height:172px; border-radius:16px 16px 10px 10px;
      background: linear-gradient(180deg, rgba(255,255,255,.05), rgba(255,255,255,.015));
      border:1.5px solid var(--border-g);
      box-shadow: inset 0 3px 14px rgba(0,0,0,.18), 0 8px 22px rgba(0,0,0,.16);
      overflow:hidden;
      display:flex; align-items:flex-end;
    }
    /* الشمع الممتلئ اللي بيرتفع حسب الدرجة */
    .candle-wax {
    ...
    .candle-wax::before {
      content:''; position:absolute; inset:0; border-radius:0 0 8px 8px;
      background:linear-gradient(180deg, rgba(255,255,255,.28), transparent 40%);
    }
    .candle-wax::after {
      content:''; position:absolute; top:-3px; left:0; right:0; height:6px; border-radius:50%;
      background:rgba(255,255,255,.35); filter:blur(2px);
    }
    /* رقم الدرجة محفور جوه الشمع */
    .candle-score {
      position:absolute; left:0; right:0; bottom:14px; text-align:center; z-index:2;
      font-family:'Aref Ruqaa','Cairo',serif; font-weight:700; font-size:26px; color:#fff;
      text-shadow:0 1px 3px rgba(0,0,0,.5);
      transition: opacity .4s ease;
    }
    /* الفتيلة + اللهب */
    .candle-wick { position:absolute; top:-16px; left:50%; transform:translateX(-50%); width:3px; height:16px; background:#3a2a18; border-radius:2px; z-index:3; }
    .candle-flame-wrap {
      position:absolute; top:-52px; left:50%; transform:translateX(-50%); width:34px; height:46px; z-index:4;
    }
    .candle-flame {
      position:absolute; inset:0; transform-origin:50% 90%;
      animation: flameDance 2.3s ease-in-out infinite;
    }
    @keyframes flameDance {
      0%,100% { transform:translateX(-50%) rotate(-3deg) scaleY(1); }
      25%     { transform:translateX(-50%) rotate(2deg) scaleY(1.05); }
      50%     { transform:translateX(-50%) rotate(-2deg) scaleY(.96); }
      75%     { transform:translateX(-50%) rotate(3deg) scaleY(1.03); }
    }
    .candle-flame-core {
      position:absolute; inset:0; transform-origin:50% 90%; left:50%; transform:translateX(-50%) scale(.55);
      animation: flameDanceCore 2.3s ease-in-out infinite reverse;
    }
    @keyframes flameDanceCore {
      0%,100% { transform:translateX(-50%) rotate(3deg) scale(.55); }
      50%     { transform:translateX(-50%) rotate(-3deg) scale(.6); }
    }
    .candle-glow-pulse {
      position:absolute; top:-70px; left:50%; transform:translateX(-50%); width:110px; height:110px; border-radius:50%;
      background:radial-gradient(circle, var(--gold-glow), transparent 70%);
      animation: glowBreathe 2.3s ease-in-out infinite; pointer-events:none; z-index:1;
    }
    @keyframes glowBreathe { 0%,100%{ opacity:.5; transform:translateX(-50%) scale(.9); } 50%{ opacity:.9; transform:translateX(-50%) scale(1.15); } }

    /* دخان متصاعد وقت التحميل */
    .candle-smoke { position:absolute; top:-60px; left:50%; width:6px; height:6px; border-radius:50%; background:rgba(180,180,190,.35); }
    .candle-smoke-piece { animation: smokeRise 2.8s ease-out infinite; opacity:0; }
    @keyframes smokeRise {
      0%   { opacity:0; transform:translate(-50%,0) scale(.4); }
      15%  { opacity:.5; }
      100% { opacity:0; transform:translate(calc(-50% + var(--sx)), -70px) scale(1.8); }
    }

    .candle-of { margin-top:16px; font-size:12px; color:var(--muted); font-weight:700; letter-spacing:.5px; }
    .candle-loading-label {
      margin-top:16px; font-size:12.5px; color:var(--muted); font-weight:700;
      animation: dotsFade 1.4s ease-in-out infinite;
    }
    @keyframes dotsFade { 0%,100%{ opacity:.5; } 50%{ opacity:1; } }

    .candle-seal {
      margin-top:18px; display:inline-flex; align-items:center; gap:8px; padding:9px 24px; border-radius:99px;
    ...
    .candle-seal svg { width:16px; height:16px; }
              ...
              <div>
                ...
                {publicStudentResult && (
                  <div style={{ animation:'fadeUp .5s cubic-bezier(.16,1,.3,1) both' }}>
                    ...
                    {/* ══ شمعة الدرجة القبطية المتقدة ══ */}
                    <div className="candle-wrap">
                      <div className="candle-halo" />
                      <div className="candle-halo-ring" />
                      {/* صلبان قبطية صغيرة حوالين الهالة */}
                      {[0,45,90,135,180,225,270,315].map((ang,i)=>(
                        <svg key={i} className="candle-cross" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"
                          style={{
                            top:`calc(50% + ${Math.sin(ang*Math.PI/180)*105}px - 6px)`,
                            left:`50%`,
                            transform:`translateX(calc(-50% + ${Math.cos(ang*Math.PI/180)*105}px))`,
                          }}>
                      ...
                      {(() => {
                        ...
                        const waxColor = publicStudentResult.grade.color || 'var(--gold)';
                        const waxHeightPct = ringReady ? pctVal : 0;
                        return (
                          <div className="candle-stage">
                            <div className="candle-glow-pulse" style={{ background:`radial-gradient(circle, ${waxColor}33, transparent 70%)` }} />
                            <div className="candle-body-wrap">
                              {/* الفتيلة */}
                              <div className="candle-wick" />
                              {/* اللهب: بيرقص دايمًا (تحميل ونتيجة) */}
                              <div className="candle-flame-wrap">
                                <svg className="candle-flame" viewBox="0 0 34 46" style={{ color: waxColor }}>
                                  <path d="M17 2C10 12 6 18 6 26a11 11 0 0 0 22 0c0-6-2-10-5-14 .5 4-1 6-3 6-3 0-4-3-3-7-2 1-4 3-4 6 0 3 2 5 4 6-4 1-8-2-8-8 0-6 4-11 8-15z"
                                    fill="currentColor" opacity=".55"/>
                                </svg>
                                <svg className="candle-flame-core" viewBox="0 0 34 46">
                                  <path d="M17 2C10 12 6 18 6 26a11 11 0 0 0 22 0c0-6-2-10-5-14 .5 4-1 6-3 6-3 0-4-3-3-7-2 1-4 3-4 6 0 3 2 5 4 6-4 1-8-2-8-8 0-6 4-11 8-15z"
                                    fill="#fff9e0"/>
                                </svg>
                              </div>
                              {/* دخان وقت التحميل بس */}
                              {!ringReady && [
                                {sx:'-14px', delay:'0s'}, {sx:'6px', delay:'.7s'}, {sx:'-4px', delay:'1.4s'},
                              ].map((s,i)=>(
                                <span key={i} className="candle-smoke candle-smoke-piece" style={{ '--sx':s.sx, animationDelay:s.delay }} />
                              ))}
                              {/* وعاء الشمعة والشمع */}
                              <div className="candle-glass">
                                <div className="candle-wax" style={{
                                  height:`${Math.max(6, waxHeightPct)}%`,
                                  background:`linear-gradient(180deg, ${waxColor}, ${waxColor}cc)`,
                                }}>
                                  {ringReady && (
                                    <span className="candle-score">{publicStudentResult.total}</span>
                                  )}
                                </div>
                              </div>
                            </div>
                            ...
                            {ringReady
                              ? <p className="candle-of">من ١٠٠ درجة</p>
                              : <p className="candle-loading-label">🕯️ الشمعة بتتقد بدرجتك...</p>}
                          </div>
                        );
                      })()}

                      <div className="candle-seal" style={{ color:publicStudentResult.grade.color, opacity: ringReady ? 1 : 0, transform: ringReady ? 'scale(1)' : 'scale(.9)' }}>
