...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .melody-endpoint-score { font-family:'Aref Ruqaa','Cairo',serif; font-weight:700; font-size:19px; color:var(--text); line-height:1; }
              ...
              <div>
                ...
                {publicStudentResult && (
                  <div style={{ animation:'fadeUp .5s cubic-bezier(.16,1,.3,1) both' }}>
                    ...
                    {/* ══ خط اللحن المتحرك: تمثيل الدرجة كنغمة بتترسم ══ */}
                    <div className="melody-wrap">
                      {(() => {
                        const pctVal = Math.max(0, Math.min(100, parseFloat(publicStudentResult.total) || 0));
                        const lineColor = publicStudentResult.grade.color || 'var(--gold)';
                        // مسار منحني بسيط (زي موجة لحن) من اليمين لليسار (RTL) بعرض المرحلة
                        const W = 264, H = 200;
                        const pathD = `M 18 ${H*0.55} C 70 ${H*0.15}, 120 ${H*0.85}, 172 ${H*0.4} S 230 ${H*0.7}, 246 ${H*0.42}`;
                        // نقاط تقريبية على طول الخط لعرض النوتات (5 نقاط موزعة)
                        const notePoints = [
                          {x:34,  y:H*0.48, delay:0},
                          {x:86,  y:H*0.32, delay:.35},
                          {x:138, y:H*0.62, delay:.7},
                          {x:190, y:H*0.5,  delay:1.05},
                          {x:236, y:H*0.46, delay:1.4},
                        ];
                        const visibleNotes = Math.ceil((pctVal/100) * notePoints.length);
                        const TRACK_LEN = 340; // تقدير طول تقريبي للمسار لأغراض الأنيميشن
                        const finalOffset = TRACK_LEN * (1 - pctVal/100);
                        return (
                          <div className="melody-stage">
                            {/* خطوط سطر النوتة الخمسة */}
                            <div className="melody-staff-lines">
                              {[0,1,2,3,4].map(i=>(
                                <span key={i} style={{ top:`${18 + i*22}%` }} />
                              ))}
                            </div>

                            <svg className="melody-svg" viewBox={`0 0 ${W} ${H}`}>
                              <path className="melody-track" d={pathD} />
                              <path
                                className={`melody-bar ${ringReady ? '' : 'is-loading'}`}
                                d={pathD}
                                style={{ color:lineColor, stroke:lineColor, '--trackLen':TRACK_LEN }}
                                strokeDasharray={TRACK_LEN}
                                strokeDashoffset={ringReady ? finalOffset : TRACK_LEN}
                              />
                            </svg>

                            {/* وضع التحميل: نوتة راكبة بتتنقل + موجة صوتية */}
                            {!ringReady && (
                              <>
                                <span className="melody-cursor" style={{ top:`${H*0.45}px` }}>♪</span>
                                <div className="melody-eq">
                                  {[0,1,2,3,4].map(i=>(
                                    <span key={i} style={{ animationDelay:`${i*0.12}s`, background:'var(--gold)' }} />
                                  ))}
                                </div>
                              </>
                            )}

                            {/* النوتات المتتالية اللي بتظهر مع رسم الخط */}
                            {ringReady && notePoints.slice(0, visibleNotes).map((p,i)=>(
                              <span key={i}
                                className={`melody-note ${i === visibleNotes-1 ? 'is-bouncing' : ''}`}
                                style={{
                                  left:`${(p.x/W)*100}%`, top:`${p.y}px`,
                                  color:lineColor, animationDelay:`${p.delay}s`,
                                }}>
                                {i % 2 === 0 ? '♪' : '♫'}
                              </span>
                            ))}

                            {/* نقطة الوصول: دائرة الرقم النهائي */}
                            <div className={`melody-endpoint ${ringReady ? 'is-ready' : ''}`}
                              style={{ left:`${(notePoints[notePoints.length-1].x/W)*100}%`, top:`${notePoints[notePoints.length-1].y}px`, color:lineColor }}>
                              <div className="melody-endpoint-halo" />
                              <span className="melody-endpoint-score">{ringReady ? publicStudentResult.total : '♪'}</span>
                            </div>
                          </div>
                        );
                      })()}
                      ...
                      {ringReady
                        ? <p className="melody-of">من ١٠٠ درجة</p>
                        : <p className="melody-loading-label">🎼 بيتكوّن لحن درجتك...</p>}
