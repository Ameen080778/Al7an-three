...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── Score medallion (circular progress) ── */
    .medal-wrap { position:relative; display:flex; flex-direction:column; align-items:center; padding:18px 10px 6px; }
    .medal-rays {
      ...
      border-radius:50%; animation:ringGrow .7s cubic-bezier(.16,1,.3,1) both;
      pointer-events:none;
    }
    .medal-dot {
    ...
    .medal-ring-track { fill:none; stroke:var(--border); stroke-width:12; }
    .medal-ring-bar {
      fill:none; stroke-width:12; stroke-linecap:round;
      transition: stroke-dashoffset 1.4s cubic-bezier(.16,1,.3,1), stroke .4s ease;
      filter: drop-shadow(0 0 8px currentColor);
    }
    .medal-inner {
      position:relative; z-index:1; width:148px; height:148px; border-radius:50%;
      display:flex; flex-direction:column; align-items:center; justify-content:center;
    ...
    .success-ticket {
      position:relative; margin-top:18px; border-radius:20px; overflow:visible;
      background:linear-gradient(120deg, #e11d48, #f97316 65%, #f5c842);
      box-shadow:0 16px 36px rgba(225,29,72,.32);
      animation:ticketDrop .55s .2s cubic-bezier(.16,1,.3,1) both;
      color:#fff; padding:22px 20px 4px;
    }
    ...
    .ticket-head { font-weight:900; font-size:18px; margin-bottom:6px; }
    .ticket-sub  { font-size:13px; opacity:.92; line-height:1.7; margin-bottom:16px; }
    .ticket-perf {
      border-top:2px dashed rgba(255,255,255,.55); position:relative; padding-top:14px; padding-bottom:16px;
    }
    ...
    .ticket-sched-label { font-size:11px; opacity:.85; margin-bottom:3px; }
    .ticket-sched-value { font-family:'Aref Ruqaa','Cairo',serif; font-weight:700; font-size:19px; }
                  ...
                  <div style={{ animation:'fadeUp .5s cubic-bezier(.16,1,.3,1) both' }}>
                    ...
                    {parseFloat(publicStudentResult.total) >= 50 && (
                      <div className="success-ticket">
                        <p className="ticket-head">🎉 مبروك.. اجتزت السنة الأولى بنجاح!</p>
                        <p className="ticket-sub">مدرسة Ⲧⲉⲛⲟⲩⲉϩ ⲛ̀ⲥⲱⲕ لتعليم الالحان بتفتحلك دلوقتي باب السنة الثانية ✨</p>
                        <div className="ticket-perf" style={{ display:'flex', alignItems:'center', gap:14 }}>
                          <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.8" style={{flexShrink:0}}><rect x="3" y="4" width="18" height="17" rx="3"/><path d="M8 2v4M16 2v4M3 9h18" strokeLinecap="round"/></svg>
                          <div>
                            <p className="ticket-sched-label">مواعيد السنة الدراسية الثانية</p>
                            <p className="ticket-sched-value">الأربعاء - ٧:٣٠ مساءً</p>
                          </div>
                        </div>
                      </div>
                    )}
