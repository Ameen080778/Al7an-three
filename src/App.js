...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ═══════════════════════════════════════════════
       SCORE CARD — كارت الدرجة + شريط تقدم أفقي متزامن مع العداد
    ═══════════════════════════════════════════════ */
    .score-card { position:relative; display:flex; flex-direction:column; align-items:center; padding:22px 10px 8px; }
    .score-card-top { display:flex; align-items:baseline; gap:6px; margin-bottom:16px; }
    .score-card-number {
      font-family:'Aref Ruqaa','Cairo',serif; font-weight:900; font-size:46px; line-height:1;
      filter: drop-shadow(0 0 10px currentColor); transition: color .3s ease;
    }
    .score-card-max { font-size:14px; font-weight:700; color:var(--muted); }
    .score-card-bar-track {
      width:100%; max-width:280px; height:14px; border-radius:99px; background:var(--border);
      overflow:hidden; position:relative; box-shadow: inset 0 1px 4px rgba(0,0,0,.15);
    }
    .score-card-bar-fill {
    ...
    .score-card-number.is-final-pulse { animation: finalNumberPulse .5s cubic-bezier(.34,1.56,.64,1) both; }

    /* بادچ الترتيب — يظهر بس لأول 10 على الفرقة */
    .rank-badge {
    ...
    .rank-badge.rank-top3 {
      background:linear-gradient(135deg, rgba(245,200,66,.16), rgba(245,200,66,.06));
      border:1px solid rgba(245,200,66,.4); color:var(--gold-d);
    }
    .rank-badge.rank-top10 {
      background:rgba(139,92,246,.08); border:1px solid rgba(139,92,246,.28); color:var(--violet);
    }
              ...
              <div>
                ...
                {publicStudentResult && (
                  <div style={{ animation:'fadeUp .5s cubic-bezier(.16,1,.3,1) both' }}>
                    ...
                    {/* ══ عرض الدرجة: شريط تقدم أفقي بيتملى مع عداد رقمي متزامن ══ */}
                        ...
                        return (
                          <>
                            <div className="score-card-top">
                              <span className={`score-card-number ${scoreCounted ? 'is-final-pulse' : ''}`} style={{ color: barColor }}>
                                {ringReady ? displayScore.toFixed(2) : '٠٠.٠٠'}
                              </span>
                              <span className="score-card-max">/ 100</span>
                            </div>
                            <div className="score-card-bar-track">
                              <div className="score-card-bar-fill" style={{ width: ringReady ? `${pctVal}%` : '0%', background: barColor, color: barColor }} />
                            </div>

                            {/* بادچ الترتيب: يظهر بس لو الطالب من ضمن أول 10 على الفرقة */}
                            {ringReady && publicStudentResult.rank && (
                              <div className={`rank-badge ${publicStudentResult.rank <= 3 ? 'rank-top3' : 'rank-top10'}`}>
                                <span className="rank-badge-emoji">
                                  {publicStudentResult.rank === 1 ? '🥇' : publicStudentResult.rank === 2 ? '🥈' : publicStudentResult.rank === 3 ? '🥉' : '🌟'}
                                </span>
                                {publicStudentResult.rank <= 3
                                  ? `المركز ${['الأول','الثاني','الثالث'][publicStudentResult.rank-1]} على الفرقة`
                                  : 'من ضمن أفضل 10 في الفرقة'}
                              </div>
                            )}
                          </>
                        );
