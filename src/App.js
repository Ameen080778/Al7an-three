            ...
            {studentResult && (
              <div style={{ animation:'slideUp .5s cubic-bezier(.16,1,.3,1) both' }}>
                ...
                {[
                  ...
                  { label:'الامتحان النهائي', icon:'🏆', data:studentResult.final, max:35, color:'var(--rose)', bg:'rgba(244,63,94,.06)', border:'rgba(244,63,94,.15)', d:'.16s',
                  ...
                  <div style={{ animation:'fadeUp .5s cubic-bezier(.16,1,.3,1) both' }}>
                    ...
                    {[
                      { label:'امتحان الشهر', icon:'📝', data:publicStudentResult.monthly, max:30, color:'#3b82f6', bg:'rgba(59,130,246,.06)', border:'rgba(59,130,246,.15)',
                        subs:[{l:'قبطي',k:'coptic',m:10},{l:'طقس',k:'liturgy',m:10},{l:'تسميع',k:'oral',m:10}] },
                      { label:'الامتحان النهائي', icon:'🏆', data:publicStudentResult.final, max:35, color:'var(--rose)', bg:'rgba(244,63,94,.06)', border:'rgba(244,63,94,.15)',
                        subs:[{l:'قبطي',k:'coptic',m:15},{l:'تسميع',k:'oral',m:20}] },
                    ].map((exam, ei) => {
                      const data = exam.data || {};
                    ...
                    {parseFloat(publicStudentResult.total) >= 50 && (
                      <div style={{ textAlign:'center', padding:'22px 18px', marginTop:14, background:'linear-gradient(135deg,rgba(220,38,38,.06),rgba(245,158,11,.06))', border:'2px solid rgba(220,38,38,.18)', borderRadius:22 }}>
                        <p style={{ fontWeight:900, fontSize:15, color:'var(--text)', marginBottom:10 }}>مدرسة Ⲧⲉⲛⲟⲩⲉϩ ⲛ̀ⲥⲱⲕ لتعليم الالحان</p>
                        <p style={{ fontSize:16, fontWeight:900, color:'#dc2626', marginBottom:10 }}>🎉 تهنئة لتفوقك 🎉</p>
                        <p style={{ fontSize:13, color:'var(--muted)', lineHeight:1.8, marginBottom:14 }}>اجتزت السنة الأولى بنجاح، ودلوقتي هتبدأ معانا رحلة جديدة في السنة الثانية ✨</p>
                        <div style={{ background:'var(--inp-bg)', borderRadius:14, padding:'14px', border:'1px solid var(--border)' }}>
                          <p style={{ fontSize:12, fontWeight:900, color:'var(--muted)', marginBottom:4 }}>📅 مواعيد السنة الدراسية الثانية</p>
