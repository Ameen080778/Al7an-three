...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ══ Result certificate (نتيجة الطالب) ══ */
    @keyframes certRise    { from { opacity:0; transform:translateY(22px) scale(.97); } to { opacity:1; transform:none; } }
    @keyframes sealPop     { 0%{ transform:scale(0) rotate(-20deg); opacity:0; } 65%{ transform:scale(1.12) rotate(3deg); opacity:1; } 100%{ transform:scale(1) rotate(0); } }
    @keyframes sweepLight  { 0%{ transform:translateX(-130%) skewX(-12deg); } 100%{ transform:translateX(230%) skewX(-12deg); } }
    @keyframes numRise     { from{ opacity:0; transform:translateY(8px); } to{ opacity:1; transform:none; } }

    .cert-frame {
      position:relative; border-radius:28px; padding:2px; margin-bottom:14px;
      background:linear-gradient(155deg, var(--border-g), transparent 45%, var(--border-g));
      animation:certRise .6s cubic-bezier(.16,1,.3,1) both;
    }
    .cert-inner {
      position:relative; overflow:hidden; text-align:center;
      border-radius:26px; padding:30px 22px 26px;
      background:var(--bg-card2); border:1px solid var(--border-g);
    }
    .cert-inner::before {
      content:''; position:absolute; inset:9px; border:1px solid var(--border-g);
      border-radius:19px; pointer-events:none;
    }
    .cert-sweep {
      position:absolute; top:0; bottom:0; width:50%; pointer-events:none;
      background:linear-gradient(100deg, transparent, rgba(255,255,255,.16), transparent);
      animation:sweepLight 1.7s .25s ease-out both;
    }
    [data-theme="light"] .cert-sweep { background:linear-gradient(100deg, transparent, rgba(255,255,255,.55), transparent); }
    .cert-corner { position:absolute; width:20px; height:20px; color:var(--gold); opacity:.5; }
    .cert-corner.tl { top:9px; left:9px; }
    .cert-corner.tr { top:9px; right:9px; transform:scaleX(-1); }
    .cert-corner.bl { bottom:9px; left:9px; transform:scaleY(-1); }
    .cert-corner.br { bottom:9px; right:9px; transform:scale(-1); }
    .cert-emblem {
      width:50px; height:50px; margin:0 auto 12px; border-radius:50%;
      display:flex; align-items:center; justify-content:center;
      background:var(--gold-glow); border:1.5px solid var(--border-g); color:var(--gold);
      animation:sealPop .7s .1s cubic-bezier(.34,1.56,.64,1) both;
    }
    .cert-greet { font-size:14px; color:var(--muted); margin-bottom:2px; }
    .cert-name { font-family:'Aref Ruqaa','Cairo',serif; font-weight:700; font-size:22px; color:var(--text); }
    .cert-rule { display:flex; align-items:center; gap:9px; margin:16px auto; max-width:200px; }
    .cert-rule::before, .cert-rule::after { content:''; flex:1; height:1px; background:var(--border-g); }
    .cert-rule svg { width:12px; height:12px; color:var(--gold); flex-shrink:0; }
    .cert-total-label { font-size:12.5px; color:var(--muted); margin-bottom:2px; }
    .cert-total { font-family:'Aref Ruqaa','Cairo',serif; font-weight:700; font-size:76px; line-height:1;
      color:var(--gold); animation:numRise .5s .35s cubic-bezier(.16,1,.3,1) both; }
    .cert-total-of { font-size:12.5px; color:var(--muted); margin:6px 0 16px; }
    .cert-seal {
      display:inline-flex; align-items:center; gap:8px; padding:8px 22px; border-radius:99px;
      border:1.5px solid currentColor; font-family:'Cairo',sans-serif; font-weight:900; font-size:14.5px;
      animation:sealPop .7s .45s cubic-bezier(.34,1.56,.64,1) both;
    }
    .cert-seal svg { width:15px; height:15px; }
              ...
              <div>
                ...
                {publicStudentResult && (
                  <div style={{ animation:'fadeUp .5s cubic-bezier(.16,1,.3,1) both' }}>
                    ...
                    {/* ══ الشهادة: الاسم + المجموع + التقدير ══ */}
                    <div className="cert-frame">
                      <div className="cert-inner">
                        <div className="cert-sweep" />
                        <svg className="cert-corner tl" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4"><path d="M12 3v7M12 14v7M3 12h7M14 12h7" strokeLinecap="round"/><circle cx="12" cy="3" r="1.3" fill="currentColor" stroke="none"/><circle cx="12" cy="21" r="1.3" fill="currentColor" stroke="none"/><circle cx="3" cy="12" r="1.3" fill="currentColor" stroke="none"/><circle cx="21" cy="12" r="1.3" fill="currentColor" stroke="none"/></svg>
                        <svg className="cert-corner tr" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4"><path d="M12 3v7M12 14v7M3 12h7M14 12h7" strokeLinecap="round"/><circle cx="12" cy="3" r="1.3" fill="currentColor" stroke="none"/><circle cx="12" cy="21" r="1.3" fill="currentColor" stroke="none"/><circle cx="3" cy="12" r="1.3" fill="currentColor" stroke="none"/><circle cx="21" cy="12" r="1.3" fill="currentColor" stroke="none"/></svg>
                        <svg className="cert-corner bl" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4"><path d="M12 3v7M12 14v7M3 12h7M14 12h7" strokeLinecap="round"/><circle cx="12" cy="3" r="1.3" fill="currentColor" stroke="none"/><circle cx="12" cy="21" r="1.3" fill="currentColor" stroke="none"/><circle cx="3" cy="12" r="1.3" fill="currentColor" stroke="none"/><circle cx="21" cy="12" r="1.3" fill="currentColor" stroke="none"/></svg>
                        <svg className="cert-corner br" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4"><path d="M12 3v7M12 14v7M3 12h7M14 12h7" strokeLinecap="round"/><circle cx="12" cy="3" r="1.3" fill="currentColor" stroke="none"/><circle cx="12" cy="21" r="1.3" fill="currentColor" stroke="none"/><circle cx="3" cy="12" r="1.3" fill="currentColor" stroke="none"/><circle cx="21" cy="12" r="1.3" fill="currentColor" stroke="none"/></svg>

                        <div className="cert-emblem">
                          <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 2v20M6 8h12" strokeLinecap="round"/></svg>
                        </div>
                        <p className="cert-greet">عامل ايه يا</p>
                        <p className="cert-name">{publicStudentResult.name}</p>

                        <div className="cert-rule">
                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M12 3v7M12 14v7M3 12h7M14 12h7" strokeLinecap="round"/></svg>
                        </div>

                        <p className="cert-total-label">✨ المجموع الكلي ✨</p>
                        <div className="cert-total">{publicStudentResult.total}</div>
                        <p className="cert-total-of">من 100 درجة</p>

                        <div className="cert-seal" style={{ color:publicStudentResult.grade.color }}>
                    ...
                    {parseFloat(publicStudentResult.total) >= 50 && (
                      <div className="cert-frame" style={{ marginTop:14 }}>
                        <div className="cert-inner" style={{ padding:'24px 20px 22px' }}>
                          <svg className="cert-corner tl" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4"><path d="M12 3v7M12 14v7M3 12h7M14 12h7" strokeLinecap="round"/></svg>
                          <svg className="cert-corner br" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4"><path d="M12 3v7M12 14v7M3 12h7M14 12h7" strokeLinecap="round"/></svg>
                          <p style={{ fontFamily:"'Aref Ruqaa','Cairo',serif", fontWeight:700, fontSize:17, color:'var(--text)', marginBottom:8 }}>مدرسة Ⲧⲉⲛⲟⲩⲉϩ ⲛ̀ⲥⲱⲕ لتعليم الالحان</p>
                          <p style={{ fontSize:15, fontWeight:900, color:'var(--rose)', marginBottom:10 }}>🎉 تهنئة لتفوقك 🎉</p>
                          <p style={{ fontSize:13, color:'var(--muted)', lineHeight:1.8, marginBottom:16 }}>اجتزت السنة الأولى بنجاح، ودلوقتي هتبدأ معانا رحلة جديدة في السنة الثانية ✨</p>
                          <div style={{ background:'var(--inp-bg)', borderRadius:14, padding:'14px', border:'1px solid var(--border)' }}>
                            <p style={{ fontSize:11.5, fontWeight:800, color:'var(--muted)', marginBottom:4 }}>📅 مواعيد السنة الدراسية الثانية</p>
                            <p style={{ fontSize:14, fontWeight:900, color:'var(--text)' }}>يوم الأربعاء - الساعة ٧:٣٠ مساءً</p>
                          </div>
                        </div>
                      </div>
                    )}
