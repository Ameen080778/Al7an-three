                  ...
                  <div style={{ textAlign:'center', animation:'fadeUp .4s cubic-bezier(.16,1,.3,1) both' }}>
                    ...
                    <div style={{ display:'flex', gap:12 }}>
                      <button onClick={async () => {
                        ...
                        try {
                          const { data:sd, error:se } = await supabase.from('students').select('id,name,age,student_code').eq('student_code', code).single();
                          if (se||!sd) { setPublicResultError('❌ الكود غير صحيح!'); setPublicSearching(false); return; }
                          const [allAtt, attRes, exRes] = await Promise.all([
                            supabase.from('attendance').select('sessions'),
                            supabase.from('attendance').select('sessions').eq('student_id',sd.id).maybeSingle(),
                          ...
                          let actualSessions=0;
                          if(allAtt.data?.length>0) for(let i=0;i<12;i++) if(allAtt.data.some(r=>Array.isArray(r.sessions)&&r.sessions[i]===true)) actualSessions++;
                          const sessions=attRes.data?.sessions||[];
                          const attended=Array.isArray(sessions)?sessions.filter(Boolean).length:0;
                          const total=actualSessions>0?actualSessions:12;
                          const attScore=((attended/total)*30).toFixed(2);
                          const ex=exRes.data||{};
                          const mRawQ=(ex.m_coptic||0)+(ex.m_liturgy||0)+(ex.m_oral||0);
                          const fRawQ=(ex.f_coptic||0)+(ex.f_liturgy||0)+(ex.f_oral||0);
                          const mBQ=ex.m_bonus||0; const fBQ=ex.f_bonus||0;
                          const baseQ=((parseFloat(attScore)+mRawQ+fRawQ)/110*100);
                          const tot=(baseQ+mBQ+fBQ).toFixed(2);
                          setPublicStudentResult({ name:sd.name,age:sd.age,attendance:attScore,attendedCount:attended,totalSessions:total,
                            monthly:{coptic:ex.m_coptic||0,liturgy:ex.m_liturgy||0,oral:ex.m_oral||0,bonus:mBQ,total:mRawQ.toFixed(1)},
                            final:{coptic:ex.f_coptic||0,liturgy:ex.f_liturgy||0,oral:ex.f_oral||0,bonus:fBQ,total:fRawQ.toFixed(1)},
                            total:tot, grade:getGrade(parseFloat(tot)) });
