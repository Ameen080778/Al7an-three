...
const GlobalStyles = () => (
  <style>{`
    @import url('https://fonts.googleapis.com/css2?family=Cairo:wght@300;400;600;700;900&family=Tajawal:wght@300;400;500;700;900&family=Aref+Ruqaa:wght@400;700&display=swap');
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── Score medallion (circular progress + celebration bg) ── */
    .medal-wrap { position:relative; display:flex; flex-direction:column; align-items:center; padding:22px 10px 6px; }
    .medal-rays {
      position:absolute; top:50%; left:50%; width:230px; height:230px;
      transform:translate(-50%,-50%);
      background:repeating-conic-gradient(from 0deg, rgba(212,160,23,.28) 0deg 3deg, transparent 3deg 16deg);
      border-radius:50%; animation:ringGrow .7s cubic-bezier(.16,1,.3,1) both, spinSlow 26s linear infinite;
      pointer-events:none;
    }
    .medal-dot {
      position:absolute; top:50%; left:50%; width:7px; height:7px; border-radius:50%;
      animation:dotBurst .9s .15s cubic-bezier(.16,1,.3,1) both;
    }
    /* Confetti burst behind the ring */
    .confetti-field { position:absolute; top:50%; left:50%; width:280px; height:280px; transform:translate(-50%,-50%); pointer-events:none; z-index:0; }
    .confetti-piece {
    ...
    @keyframes confettiFloat {
      ...
      12%  { opacity:1; }
      50%  { opacity:.9; transform:translate(var(--cx), var(--cy)) rotate(var(--crot)) scale(1); }
      100% { opacity:0; transform:translate(calc(var(--cx) * 1.5), calc(var(--cy) * 1.5 + 20px)) rotate(calc(var(--crot) * 2)) scale(.7); }
    }
    @keyframes spinSlow { to { transform:translate(-50%,-50%) rotate(360deg); } }
    .medal-ring {
      position:relative; width:194px; height:194px; border-radius:50%; z-index:1;
      display:flex; flex-direction:column; align-items:center; justify-content:center;
      animation:popIn .65s cubic-bezier(.34,1.56,.64,1) both;
    }
    .medal-ring-svg { position:absolute; inset:0; width:100%; height:100%; transform:rotate(-90deg); }
    .medal-ring-track { fill:none; stroke:var(--border); stroke-width:11; }
    .medal-ring-bar {
      fill:none; stroke-width:11; stroke-linecap:round; transform-origin:95px 95px;
      transition: stroke-dashoffset 1.6s cubic-bezier(.16,1,.3,1), stroke .4s ease;
      filter: drop-shadow(0 0 9px currentColor);
    }
    .medal-ring-glow {
      fill:none; stroke-width:16; stroke-linecap:round; opacity:.28; filter:blur(4px); transform-origin:95px 95px;
      transition: stroke-dashoffset 1.6s cubic-bezier(.16,1,.3,1), stroke .4s ease;
    }
    /* ── Loading phase: spinning indeterminate arc + pulsing glow ── */
    .medal-ring-svg.is-loading .medal-ring-bar,
    .medal-ring-svg.is-loading .medal-ring-glow {
      animation: ringSpin 1s linear infinite;
    }
    .medal-ring-svg.is-loading .medal-ring-glow { animation: ringSpin 1s linear infinite, glowPulse 1.1s ease-in-out infinite; }
    @keyframes ringSpin {
      from { transform: rotate(0deg); }
      to   { transform: rotate(360deg); }
    }
    @keyframes glowPulse { 0%,100%{ opacity:.22; } 50%{ opacity:.45; } }
    .medal-inner {
      ...
      box-shadow: inset 0 2px 12px rgba(0,0,0,.14), 0 2px 18px rgba(0,0,0,.08);
      transition: box-shadow .4s ease;
    }
    .medal-total { font-family:'Aref Ruqaa','Cairo',serif; font-weight:700; font-size:48px; line-height:1; color:var(--text); animation:numRise .5s .3s cubic-bezier(.16,1,.3,1) both; position:relative; z-index:1; }
    .medal-loading-label {
      font-size:12.5px; color:var(--muted); font-weight:700; position:relative; z-index:1;
      animation: dotsFade 1.4s ease-in-out infinite;
    }
    @keyframes dotsFade { 0%,100%{ opacity:.5; } 50%{ opacity:1; } }
    .medal-of { font-size:11.5px; color:var(--muted); margin-top:3px; position:relative; z-index:1; }
    .medal-seal {
      margin-top:14px; display:inline-flex; align-items:center; gap:7px; padding:8px 22px; border-radius:99px;
      border:1.5px solid currentColor; font-weight:900; font-size:14.5px;
      animation:popIn .6s .5s cubic-bezier(.34,1.56,.64,1) both;
    }
    .medal-seal svg { width:15px; height:15px; }

    /* ── Success ticket (تهنئة + مواعيد) ── */
    .success-ticket {
      position:relative; margin-top:20px; border-radius:22px; overflow:visible;
      background: linear-gradient(135deg, #ff8a65 0%, #ff6f91 35%, #ffb74d 70%, #ffd76a 100%);
      box-shadow:0 18px 40px rgba(255,111,145,.32), 0 0 0 1px rgba(255,255,255,.25) inset;
      animation:ticketDrop .55s .2s cubic-bezier(.16,1,.3,1) both;
      color:#fff; padding:26px 22px 6px; text-align:center;
    }
    [data-theme="light"] .success-ticket {
      background: linear-gradient(135deg, #ff9e7d 0%, #ff7aa2 35%, #ffc266 70%, #ffe08a 100%);
      box-shadow:0 18px 40px rgba(255,122,162,.28), 0 0 0 1px rgba(255,255,255,.3) inset;
    }
    .success-ticket::before, .success-ticket::after {
      content:''; position:absolute; top:50%; width:24px; height:24px; background:var(--bg);
      border-radius:50%; transform:translateY(-50%); z-index:1;
    }
    .success-ticket::before { left:-12px; }
    .success-ticket::after  { right:-12px; }
    .ticket-school {
      font-family:'Aref Ruqaa','Cairo',serif; font-weight:700; font-size:15px; letter-spacing:.3px;
      color:#fff; text-shadow:0 1px 6px rgba(0,0,0,.12); opacity:.98; margin-bottom:14px; line-height:1.6;
    }
    .ticket-head {
      font-weight:900; font-size:22px; margin-bottom:10px; color:#fff;
      text-shadow:0 2px 10px rgba(0,0,0,.15);
    }
    .ticket-sub  { font-size:13.5px; opacity:.98; line-height:1.9; margin-bottom:18px; font-weight:600; color:#fff; text-shadow:0 1px 6px rgba(0,0,0,.1); }
    .ticket-perf {
      border-top:2px dashed rgba(255,255,255,.55); position:relative; padding-top:16px; padding-bottom:18px;
      display:flex; align-items:center; justify-content:center; gap:14px;
    }
    .ticket-perf::before, .ticket-perf::after {
      content:''; position:absolute; top:-13px; width:22px; height:22px; background:var(--bg);
      border-radius:50%;
    }
    ...
    .ticket-sched-label { font-size:11.5px; opacity:.95; margin-bottom:4px; font-weight:700; color:#fff; }
    .ticket-sched-value { font-family:'Aref Ruqaa','Cairo',serif; font-weight:700; font-size:20px; color:#fff; text-shadow:0 1px 6px rgba(0,0,0,.12); }
    /* Floating celebration particles inside the ticket */
    .ticket-sparkle {
      position:absolute; pointer-events:none; opacity:0;
      animation: sparkleRise 2.6s ease-in-out infinite;
    }
              ...
              <div>
                ...
                {publicStudentResult && (
                  <div style={{ animation:'fadeUp .5s cubic-bezier(.16,1,.3,1) both' }}>
                    ...
                    {/* ══ الميدالية: المجموع + التقدير ══ */}
                    <div className="medal-wrap">
                      <div className="medal-rays" />
                      {/* Confetti burst */}
                      <div className="confetti-field">
                        {[
                          {a:-80,d:130,c:'var(--gold)',s:'8px',r:'square'},
                          {a:-50,d:150,c:'var(--rose)',s:'6px',r:'circle'},
                          {a:-20,d:140,c:'var(--teal)',s:'7px',r:'square'},
                          {a:15,d:155,c:'var(--violet)',s:'6px',r:'circle'},
                          {a:50,d:135,c:'var(--gold)',s:'7px',r:'square'},
                          {a:85,d:150,c:'var(--rose)',s:'6px',r:'circle'},
                          {a:120,d:140,c:'var(--teal)',s:'8px',r:'square'},
                          {a:155,d:130,c:'var(--gold)',s:'6px',r:'circle'},
                          {a:190,d:150,c:'var(--violet)',s:'7px',r:'square'},
                          {a:225,d:140,c:'var(--rose)',s:'6px',r:'circle'},
                          {a:260,d:155,c:'var(--gold)',s:'8px',r:'square'},
                          {a:300,d:135,c:'var(--teal)',s:'6px',r:'circle'},
                        ].map((p,i)=>(
                          <span key={i} className="confetti-piece" style={{
                            width:p.s, height:p.s, background:p.c,
                            borderRadius: p.r==='circle' ? '50%' : '2px',
                            '--cx':`${Math.cos(p.a*Math.PI/180)*p.d}px`,
                            '--cy':`${Math.sin(p.a*Math.PI/180)*p.d}px`,
                            '--crot':`${180+i*30}deg`,
                      ...
                      {[
                        {a:-70,c:'var(--gold)'},{a:-35,c:'var(--rose)'},{a:0,c:'var(--teal)'},{a:35,c:'var(--gold)'},
                        {a:70,c:'var(--rose)'},{a:110,c:'var(--teal)'},{a:145,c:'var(--gold)'},{a:180,c:'var(--rose)'},
                        {a:215,c:'var(--teal)'},{a:250,c:'var(--gold)'},
                      ].map((d,i)=>(
                        <span key={i} className="medal-dot" style={{
                          background:d.c,
                          '--dx':`${Math.cos(d.a*Math.PI/180)*105}px`,
                          '--dy':`${Math.sin(d.a*Math.PI/180)*105}px`,
                          animationDelay:`${.15+i*.03}s`,
                        }}/>
                      ))}
                      {(() => {
                        const R = 83;
                        ...
                        const finalOffset = CIRC * (1 - pctVal / 100);
                        const ringColor = publicStudentResult.grade.color || 'var(--gold)';
                        // وضع التحميل: قوس جزئي ثابت الطول (25% من المحيط) بيدور بلا توقف
                        const loadingOffset = CIRC * 0.75;
                        const offset = ringReady ? finalOffset : loadingOffset;
                        return (
                          <div className="medal-ring">
                            <svg
                              className={`medal-ring-svg ${ringReady ? '' : 'is-loading'}`}
                              viewBox="0 0 190 190"
                              style={{ '--circ': `${CIRC}px` }}
                            >
                              <circle className="medal-ring-track" cx="95" cy="95" r={R} />
                              <circle
                                className="medal-ring-glow"
                                cx="95" cy="95" r={R}
                                style={{ color: ringColor, stroke: ringColor }}
                                strokeDasharray={CIRC}
                                strokeDashoffset={offset}
                              />
                              <circle
                                className="medal-ring-bar"
                                cx="95" cy="95" r={R}
                                style={{ color: ringColor, stroke: ringColor }}
                                strokeDasharray={CIRC}
                                strokeDashoffset={offset}
                            ...
                            <div className="medal-inner">
                              {ringReady ? (
                                <>
                                  <div className="medal-total">{publicStudentResult.total}</div>
                                  <p className="medal-of">من 100 درجة</p>
                                </>
                              ) : (
                                <>
                                  <div className="medal-total" style={{ fontSize:26 }}>⏳</div>
                                  <p className="medal-loading-label">بنحسب درجتك...</p>
                                </>
                              )}
                            </div>
                          </div>
                      ...
                      <div className="medal-seal" style={{ color:publicStudentResult.grade.color, opacity: ringReady ? 1 : 0, transform: ringReady ? 'scale(1)' : 'scale(.9)', transition:'opacity .4s ease, transform .4s ease' }}>
                    ...
                    {/* ══ تذكرة النجاح: تهنئة + مواعيد السنة الثانية ══ */}
                    {parseFloat(publicStudentResult.total) >= 50 && (
                      <div className="success-ticket">
                        {/* Floating sparkles */}
                        {[
                          {l:'10%', delay:'0s',  c:'#ffe9a8'},
                          {l:'28%', delay:'.5s', c:'#f5c842'},
                          {l:'50%', delay:'1s',  c:'#ffffff'},
                          {l:'72%', delay:'1.5s',c:'#f5c842'},
                          {l:'90%', delay:'.8s', c:'#ffe9a8'},
                        ].map((s,i)=>(
                          <span key={i} className="ticket-sparkle" style={{ left:s.l, bottom:'14px', animationDelay:s.delay, fontSize:14, color:s.c }}>✦</span>
                        ))}
                        <p className="ticket-school">مدرسة Ⲧⲉⲛⲟⲩⲉϩ ⲛ̀ⲥⲱⲕ لتعليم الالحان</p>
                        <p className="ticket-head">🎉 تهنئك لتفوقك 🎉</p>
                        <p className="ticket-sub">اجتزت السنة الأولى بنجاح، ودلوقتي هتبدأ معانا رحلة جديدة في السنة الثانية ✨</p>
                        <div className="ticket-perf">
                          <div className="ticket-sched-icon">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f5d98a" strokeWidth="1.8"><rect x="3" y="4" width="18" height="17" rx="3"/><path d="M8 2v4M16 2v4M3 9h18" strokeLinecap="round"/></svg>
                          </div>
                          <div style={{ textAlign:'right' }}>
                            <p className="ticket-sched-label">📅 مواعيد السنة الدراسية الثانية</p>
                            <p className="ticket-sched-value">يوم الأربعاء - الساعة ٧:٣٠ مساءً</p>
                          </div>
                        </div>
                      </div>
                    )}

                    {/* ══ تفاصيل الدرجات (قابلة للطي) ══ */}
                    ...
                    {showResultDetails && (
                      <div style={{ animation:'fadeUp .35s cubic-bezier(.16,1,.3,1) both' }}>
                        ...
                        {[
                          ...
                          const breakdownText = exam.showBreakdown !== false
                            ? (exam.subs||[]).map(sub=>`${sub.l} ${parseFloat(data[sub.k]||0).toFixed(1)}`).join('  ·  ')
                            : null;
                          return (
                            <div key={ei} className="ledger-row" style={{ borderInlineStart:`3px solid ${exam.color}` }}>
                              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:breakdownText?5:8 }}>
                              ...
                              {breakdownText && <p style={{ fontSize:10.5, color:'var(--muted)', marginBottom:8 }}>{breakdownText}</p>}
                              <div style={{ height:4, background:'var(--border)', borderRadius:99, overflow:'hidden' }}>
                                <div style={{ height:'100%', borderRadius:99, background:exam.color, width:`${pct}%`, transition:'width 1s .3s cubic-bezier(.16,1,.3,1)' }}/>
                              </div>
                            </div>
