                                  ...
                                  <div style={{ display:'flex', gap:8, marginTop:12, overflowX:'auto' }}>
                                    {[chant.image_url, chant.image_url_2].filter(Boolean).map((img, idx) => (
                                      <button key={idx} onClick={() => setLightboxImage(driveImageUrl(img))}
                                        style={{ flex: chant.image_url && chant.image_url_2 ? '0 0 48%' : '0 0 100%', padding:0, border:'1px solid var(--border-g)', borderRadius:12, overflow:'hidden', cursor:'zoom-in', background:'var(--inp-bg)' }}>
